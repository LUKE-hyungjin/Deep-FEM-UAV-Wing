...
# Use demo_data for deployment (smaller, only GLB + JSON)
DEMO_MODE = (PROJECT_ROOT / "demo_data").exists()
DATA_ROOT = PROJECT_ROOT / "demo_data" if DEMO_MODE else PROJECT_ROOT / "data" / "raw"

GEOMETRY_DIR = DATA_ROOT / "geometry"
FEM_DIR = DATA_ROOT / "fem"
MESH_DIR = PROJECT_ROOT / "data" / "raw" / "mesh"  # Only needed for mesh report (optional)
CHECKPOINTS_DIR = DATA_ROOT / "checkpoints" if DEMO_MODE else PROJECT_ROOT / "checkpoints"
...
def _list_success_cases() -> list[dict[str, str]]:
    """List available cases by scanning FEM directory."""
    ...
    for case_dir in sorted(FEM_DIR.iterdir()):
        ...
        # Check if required GLB exists
        if not (case_dir / "wing_result.glb").exists():
            continue

        # Load params if available
        params = _load_geometry_params(case_id) or {}
        rows.append({
            "case_id": case_id,
            "span_m": str(params.get("span_m", "")),
            "chord_m": str(params.get("chord_m", "")),
            "sweep_deg": str(params.get("sweep_deg", "")),
            "thickness_ratio": str(params.get("thickness_ratio", "")),
            "status": "success",
        })
